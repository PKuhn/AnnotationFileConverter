...
public class FileParser {
    ...
    public void parseAnnotationFilesInDirectory(String path) {
        ...
        fileNames.stream().filter(fileName -> fileName.contains(".txt")
            ...
            if (fileName.equals("54c7be6de4b0554ac3c8a665.txt")) {
                System.out.println("");
            }
    ...
    */
    ...
    private List<AnnotationEntity> sortAnnotations(List<AnnotationEntity>) {
    ...
    private List<Integer> findStartingPositionsOfTokens(List<String> tokens, String text) {
        ...
        List<String> processedTokens = preprocessTokens(tokens);
        List<Integer> startPositions = new ArrayList<Integer>();

        while (tokenIndex < processedTokens.size()) {

            String nextToken = processedTokens .get(tokenIndex);
    ...
    /**
     * helper method that reads in the text and returns it as string with lines separeted by " "
     * @param textName name of the text which should be searched
     * @param path directory in which the text should be searched
     * @return text in given text file with lines separeted by " "
     */
